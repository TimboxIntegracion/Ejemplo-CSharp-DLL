# C# 

Una vez agregada la librería y registrada correctamente se utiliza de la siguiente manera:
````
 using TimboxIntegracion.TimboxLib;
````  
## Timbrado
Para utilizar los métodos de timbrado es necesario crear una instancia de la clase TimboxWebServicesTimbrado
 de la siguiente manera:
 
````
TimboxWebServicesTimbrado createrequestTimbrado = Create.createTimbradoRequest(Create.TYPE_TIMBRADO, Create.ENVIRONMENT_STAGING);
````

Para la utilización de cualquier método debe haber una configuración previa, la cual contiene los datos con los que se realizara la petición: 

````
TimboxWebServicesData config = new TimboxWebServicesData();
````

### Timbrar CFDI
Para utilizar el método de timbrar_cfdi de nuestro web services se deberá realizar la siguiente configuración:

````
TimboxWebServicesData config = new TimboxWebServicesData();

config.username = "AAA010101000";
config.password = "h6584D56fVdBbSmmnB";
config.xml = "";

Console.WriteLine(createrequestTimbrado.timbrar(config));
````

### Timbrar CFDI Referencia
Para utilizar el método de timbrar_cfdi_referencia de nuestro web services se deberá realizar la siguiente configuración:

````
TimboxWebServicesData config = new TimboxWebServicesData();

config.username = "AAA010101000";
config.password = "h6584D56fVdBbSmmnB";
config.xml = "";
config.external_id = "123456789";

Console.WriteLine(createrequestTimbrado.timbrar_referencia(config));
````
### Timbrar ZIP
Para utilizar el método de timbrar_zip de nuestro web services se deberá realizar la siguiente configuración:
````
TimboxWebServicesData config = new TimboxWebServicesData();

config.username = "AAA010101000";
config.password = "h6584D56fVdBbSmmnB";
config.zipbase64 = "";

Console.WriteLine(createrequestTimbrado.timbrar_zip(config));
````
### Buscar CFDIs
Para utilizar el método de buscar_cfdis de nuestro web services se deberá realizar la siguiente configuración:

````
TimboxWebServicesData config = new TimboxWebServicesData();

config.username = "AAA010101000";
config.password = "h6584D56fVdBbSmmnB";
config.rfc_emisor = ""; // Opcional
config.rfc_receptor = ""; // Opcional
config.fecha_emision_inicio = ""; // Opcional
config.fecha_emision_fin = ""; // Opcional
config.fecha_timbrado_inicio = ""; // Opcional
config.fecha_timbrado_fin = ""; // Opcional
config.cancelado = ""; // Opcional
config.external_id = ""; // Opcional
config.uuid_id = "406F17E0-C70B-48C3-BF82-5B2C72D0BCEE";
config.folioid = ""; // Opcional
config.serie = ""; // Opcional
config.limit = ""; // Opcional
config.offset = ""; // Opcional

Console.WriteLine(createrequestTimbrado.buscar_cfdi(config));
````
### Buscar Acuse Recepcion
Para utilizar el método de buscar_acuse_recepcion de nuestro web services se deberá realizar la siguiente configuración:

````
TimboxWebServicesData config = new TimboxWebServicesData();

config.username = "AAA010101000";
config.password = "h6584D56fVdBbSmmnB";

// Puede ser configurado un solo uuid de la siguiente manera:
TimboxUuid uuid_id = new TimboxUuid();
uuid_id.uuid = "FFBBA8A8-A1D3-4E3A-9FC4-C883B1A91B712";
config.uuid = uuid_id;

// Puede ser configurado una lista uuids de la siguiente manera:
List<TimboxUuid> lista_uuid_buscar = new List<TimboxUuid>();
lista_uuid_buscar.Add(new TimboxUuid() { uuid = "406F17E0-C70B-48C3-BF82-5B2C72D0BCEE" });
lista_uuid_buscar.Add(new TimboxUuid() { uuid = "FFBBA8A8-A1D3-4E3A-9FC4-C883B1A91B712" });
config.uuids = lista_uuid_buscar.ToArray();
config.fecha_timbrado_inicio = ""; // Opcional
config.fecha_timbrado_fin = ""; // Opcional

Console.WriteLine(createrequestTimbrado.buscar_acuse_recepcion(config));
````
### Recuperar Comprobante
Para utilizar el método de recuperar_comprobante de nuestro web services se deberá realizar la siguiente configuración:

````
TimboxWebServicesData config = new TimboxWebServicesData();

config.username = "AAA010101000";
config.password = "h6584D56fVdBbSmmnB";

// Puede ser configurado un solo uuid de la siguiente manera:
TimboxUuid uuid_id_recuperar = new TimboxUuid();
uuid_id_recuperar.uuid = "406F17E0-C70B-48C3-BF82-5B2C72D0BCEE";
config.uuid = uuid_id_recuperar;

// Puede configurar una lista uuids de la siguiente manera:
List<TimboxUuid> lista_uuid_recuperar = new List<TimboxUuid>();
lista_uuid_recuperar.Add(new TimboxUuid() { uuid = "406F17E0-C70B-48C3-BF82-5B2C72D0BCEE" });
lista_uuid_recuperar.Add(new TimboxUuid() { uuid = "FFBBA8A8-A1D3-4E3A-9FC4-C883B1A91B71" });
config.uuids = lista_uuid_recuperar.ToArray();

Console.WriteLine(createrequestTimbrado.recuperar_comprobante(config));
````

### Recuperar Comprobante Referencia
Para utilizar el método de recuperar_comprobante_referencia de nuestro web services se deberá realizar la siguiente configuración:

````
TimboxWebServicesData config = new TimboxWebServicesData();

config.username = "AAA010101000";
config.password = "h6584D56fVdBbSmmnB";

// Puede ser configurado un solo folio de la siguiente manera:
TimboxExternal external_id_recuperar = new TimboxExternal();
external_id_recuperar.external_id = "123456789";
config.external = external_id_recuperar;

// Puede ser configurado una lista de folios la siguiente manera:
List<TimboxExternal> lista_externals = new List<TimboxExternal>();
lista_externals.Add(new TimboxExternal() { external_id = "123456789" });
lista_externals.Add(new TimboxExternal() { external_id = "123456789" });
config.externals = lista_externals.ToArray();

Console.WriteLine(createrequestTimbrado.recuperar_comprobante_referencia(config));
````

### Obtener Consumo
Para utilizar el método de obtener_consumo de nuestro web services se deberá realizar la siguiente configuración:

````
TimboxWebServicesData config = new TimboxWebServicesData();

config.username = "AAA010101000";
config.password = "h6584D56fVdBbSmmnB";

Console.WriteLine(createrequestTimbrado.obtener_consumo(config));
````

### Consultar RFC
Para utilizar el método de consulta_rfc de nuestro web services se deberá realizar la siguiente configuración:

````
TimboxWebServicesData config = new TimboxWebServicesData();

config.username = "AAA010101000";
config.password = "h6584D56fVdBbSmmnB";
config.rfc = "AAA010101AAA";

Console.WriteLine(createrequestTimbrado.consultar_rfc(config));
````

## Cancelacion
 Para utilisar los métodos de cancelacion es necesario crear una instancia de la clase TimboxWebServicesCancelacion de la siguiente manera:
 
````
 TimboxWebServicesCancelacion createrequestCancelacion = Create.createCancelacionRequest(Create.TYPE_TIMBRADO, Create.ENVIRONMENT_STAGING);
````

Para la utilizacion de cualquier metodo debe haber una configuración previa, la cual contiene los datos con los que se realizara la petición: 

````
  TimboxWebServicesData config = new TimboxWebServicesData();
````

### Cancelar CFDI
Para utilizar el método de cancelar_cfdi de nuestro web services se deberá realizar la siguiente configuración:

````
TimboxWebServicesData config = new TimboxWebServicesData();

config.username = "AAA010101000";
config.password = "h6584D56fVdBbSmmnB";            
config.rfc_emisor = "AAA010101AAA";

// Puede ser configurado un solo folio de la siguiente manera:
TimboxFolio folio = new TimboxFolio();
folio.uuid = "406F17E0-C70B-48C3-BF82-5B2C72D0BCEE";
folio.total = "7261.60";
folio.rfc_receptor = "IAD121214B34";
config.folio = folio;

// Puede ser condigurado una lista de folios de la siguiente manera:
List<TimboxFolio> lista_folio = new List<TimboxFolio>();
lista_folio.Add(new TimboxFolio() { uuid = "406F17E0-C70B-48C3-BF82-5B2C72D0BCEE", rfc_receptor = "IAD121214B34", total = "7261.60" });
lista_folio.Add(new TimboxFolio() { uuid = "FFBBA8A8-A1D3-4E3A-9FC4-C883B1A91B71", rfc_receptor = "IAD121214B34", total = "7261.60" });
config.folios = lista_folio.ToArray();

config.cert_pem = File.ReadAllText("C:\\Users\\Current User\\Your Files\\File.cer.pem");
config.llave_pem = File.ReadAllText("C:\\Users\\Current User\\Your Files\\File.key.pem");

Console.WriteLine(createrequestCancelacion.cancelar(config));
````

### Consultar Estatus 
Para utilizar el método de consultar_estatus de nuestro web services se deberá realizar la siguiente configuración:

````
TimboxWebServicesData config = new TimboxWebServicesData();

config.username = "AAA010101000";
config.password = "h6584D56fVdBbSmmnB";
config.rfc_emisor = "AAA010101AAA";
config.rfc_receptor = "IAD121214B34";
config.uuid_id = "406F17E0-C70B-48C3-BF82-5B2C72D0BCEE";
config.total = "7261.60";

Console.WriteLine(createrequestCancelacion.consultar_status(config));
````
### Consultar Documento Relacionado
Para utilizar el método de consultar_documento_relacionado de nuestro web services se deberá realizar la siguiente configuración:

````
TimboxWebServicesData config = new TimboxWebServicesData();

config.username = "AAA010101000";
config.password = "h6584D56fVdBbSmmnB";
config.uuid_id = "6E5BC378-6EDE-461F-8ABB-F941E19DF4D6";
config.rfc_receptor = "AAA010101AAA";
config.cert_pem = File.ReadAllText("C:\\Users\\Current User\\Your Files\\File.cer.pem");
config.llave_pem = File.ReadAllText("C:\\Users\\Current User\\Your Files\\File.key.pem");

Console.WriteLine(createrequestCancelacion.consultar_documento_relacionado(config));
````
### Consultar Peticiones Pendientes
Para utilizar el método de consultar_peticiones_pendientes de nuestro web services se deberá realizar la siguiente configuración:

````
TimboxWebServicesData config = new TimboxWebServicesData();

config.username = "AAA010101000";
config.password = "h6584D56fVdBbSmmnB";
config.rfc_receptor = "AAA010101AAA";
config.cert_pem = File.ReadAllText("C:\\Users\\Current User\\Your Files\\File.cer.pem");
config.llave_pem = File.ReadAllText("C:\\Users\\Current User\\Your Files\\File.key.pem");

Console.WriteLine(createrequestCancelacion.consultar_peticiones_pendientes(config));

````
### Procesar Respuesta
Para utilizar el método de procesar_respuesta de nuestro web services se deberá realizar la siguiente configuración:

````
TimboxWebServicesData config = new TimboxWebServicesData();

config.username = "AAA010101000";
config.password = "h6584D56fVdBbSmmnB";
config.rfc_receptor = "AAA010101AAA";

// Puede ser configurado un solo folio_respuesta de la siguiente manera:
TimboxRespuesta procesar_respuesta = new TimboxRespuesta();
procesar_respuesta.uuid = "406F17E0-C70B-48C3-BF82-5B2C72D0BCEE";
procesar_respuesta.total = "7261.60";
procesar_respuesta.rfc_emisor = "AAA010101AAA";
procesar_respuesta.respuesta = "A";
config.respuesta = procesar_respuesta;

// Puede ser configurado una lista de folios_respuesta de la siguiente manera:
List<TimboxRespuesta> lista_procesar_respuesta = new List<TimboxRespuesta>();
lista_procesar_respuesta.Add(new TimboxRespuesta() { uuid = "406F17E0-C70B-48C3-BF82-5B2C72D0BCEE", rfc_emisor = "AAA010101AAA", total = "7261.60", respuesta = "A" });
lista_procesar_respuesta.Add(new TimboxRespuesta() { uuid = "FFBBA8A8-A1D3-4E3A-9FC4-C883B1A91B71", rfc_emisor = "AAA010101AAA", total = "7261.60", respuesta = "R" });
config.respuestas = lista_procesar_respuesta.ToArray();
config.cert_pem = File.ReadAllText("C:\\Users\\Current User\\Your Files\\File.cer.pem");
config.llave_pem = File.ReadAllText("C:\\Users\\Current User\\Your Files\\File.key.pem"); 

Console.WriteLine(createrequestCancelacion.procesar_respuesta(config));
````
